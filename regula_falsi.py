...
def print_bolzano(tex, lower_bound, upper_bound, total_iteration, decimal_digits):
    ...
    for i in range(total_iteration):
        f_x1, f_x2 = f(x_1), f(x_2)
        x_3 = (x_1 * f_x2 - x_2 * f_x1) / (f_x2 - f_x1)
        f_x3 = f(x_3)

        table_rows += (
            f"{i+1} & "
            f"\\num{{{x_1:.{decimal_digits}f}}}".replace('.', ',') + " & "
            f"\\num{{{x_2:.{decimal_digits}f}}}".replace('.', ',') + " & "
            f"\\num{{{x_3:.{decimal_digits}f}}}".replace('.', ',') + " & "
            f"\\num{{{f_x1:.{decimal_digits}f}}}".replace('.', ',') + " & "
            f"\\num{{{f_x2:.{decimal_digits}f}}}".replace('.', ',') + " & "
            f"\\num{{{f_x3:.{decimal_digits}f}}}".replace('.', ',') + "\\\\" + "\n"
        )
